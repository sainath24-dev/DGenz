...
interface WeatherGradientCardProps {
  temperature?: string;
  location?: string;
...
export const WeatherGradientCard: React.FC<WeatherGradientCardProps> = ({
  temperature = '24°',
  ...
  return (
    <div className={`relative w-[342px] h-[184px] p-5 text-white flex flex-col justify-between overflow-hidden rounded-2xl shadow-xl ${className}`}>
      {/* Curved Gradient SVG Background */}
      <svg 
        fill="none" 
        viewBox="0 0 342 175" 
      ...
      {/* Cloud & Lightning SVG Illustration */}
      ...
      <p className="text-5xl font-black relative z-10">{temperature}</p>

      {/* Info Bottom Bar */}
      <div className="flex justify-between items-end relative z-10 text-sm">
        <div>
          <p className="text-slate-300 text-xs">{highLow}</p>
          <p className="font-semibold">{location}</p>
        </div>
        <p className="font-medium text-slate-200">{weatherCondition}</p>
      </div>
    </div>
  );
};

export default WeatherGradientCard;
